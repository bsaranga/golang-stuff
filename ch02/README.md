...
- **`rune` is `int32`, but a string's length in `len(s)` is its byte count, not rune count.** `len("é")` is 2, not 1. You'll feel this in L03; for now, just be aware that indexing a string gives bytes.
